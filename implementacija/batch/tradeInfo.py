...
df = df.withColumn("avg_quote_asset_volume", F.avg(
    "quote_asset_volume").over(window))

df = df.withColumn("total_quote_asset_volume", F.sum(
    "quote_asset_volume").over(window))

result_df = df.select("date", "pairid", "total_num_trades", "avg_num_trades",  "total_trade_volume", "avg_trade_volume", "total_quote_asset_volume", "avg_quote_asset_volume") \
              .distinct() \
              .orderBy(F.desc("date"))

print("Picked up data frame...")

result_df.write.mode("overwrite").saveAsTable("daily_trading_info")
